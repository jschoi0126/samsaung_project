...
class Preprocessor:
    def __init__(self, uid):
        ...
        app_begin_time = None
        app_last_time = None
        writer.writerow(['user', 'app', 'app_id', 'use_time_ms', 'start_time', 'weekday', 'is_weekend', 'class_seq_size', 'num_unique_class'])
        for line in reader:

            if line[0] == 'screen_off':
                usage_time = int(app_last_time) - app_begin_time
                writer.writerow([uid, last_app, abs(hash(last_app)), usage_time, app_begin_time, get_weekday(app_begin_time), get_weekday(app_begin_time) >= 5 and 1 or 0, len(classes), len(set(classes))])
